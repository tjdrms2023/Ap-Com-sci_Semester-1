...
public class gameFrametest extends JFrame implements KeyListener,Runnable
{
	...
	{ 
		...
		width = 800; // width of the frame
		height = 900; // height of the frame
...
//   ->While mutiple threads would be helpful in executing many processes, but it might cause lags.
	...
	public void MissileProcess() //Method with shooting missiles
	{
		//How it works: If the user presses space key, this method creates a new missile on (x,y)
		//and put that missile on MissList.
		if(KeySpace)
		{
			ms = new Missile(x,y);
			MissList.add(ms);
	...
	{
		DrawChar();
		DrawMissile(); //creates a missile on the frame.
	...
	//method which creates a missile.
	public void DrawMissile()
	{
		for(int i=0; i < MissList.size(); i++)
		{
			//Locations where missiles at.
			ms=(Missile)(MissList.get(i));
			
			//Same as drawImage; to prevent double buffering
			buffg.drawImage(missileImage,ms.pos.x+ 350, ms.pos.y+750, this);

			// move is from gametest; set the speed of missile.
			ms.move();
			
			//if missiles go out of the frame, missile is removed.
			if(ms.pos.x> width)
			{
				MissList.remove(i);
			}
